    ...
    for task_array in tasks:
        for task in task_array["LuckyCode"]:
            if task["batchID"] == batch_id:
                all_completed = all(
                    t.get("status") == "completed" for t in task_array["LuckyCode"]
                )
                if all_completed:
                    # print(f"Batch {batch_id} is complete")
                    event_emitter.emit(
                        "batch_complete", batch_id, f"Batch {batch_id} is complete"
                    )
                    is_complete = True
                break
        if is_complete:
    ...
    if ID is not None:
        ...
        if check_if_batch_is_complete(ID):
            if tasks_index < len(tasks) - 1:
                ...
                event_emitter.emit(
                    "message", "all tasks complete waiting for new ones..."
                )
    else:
...
async def log_requests(request: Request, call_next):
    if request.method == "GET":
        ...
        event_emitter.emit(
            "firehose", {"params": query_params, "type": "GET", "url": request.url}
        )
    elif request.method == "POST":
        body = await request.body()
        event_emitter.emit(
            "firehose", {"body": body.decode(), "type": "POST", "url": request.url}
        )
...
async def handle_hit(request: Request):
    ...
    return JSONResponse(
        content={"status": "success", "message": "Hit event received", "data": count},
        status_code=200,
    )
    # try:
    #     json_data = await request.json()
