...
def download_latest_build():
    ...
    matches = [
        ...
        if a["href"].startswith(f"luckyrobots-{current_os}-")
    ]
    ...
    sorted_files = sorted(
        matches,
        key=lambda x: datetime.strptime(x.split("-")[2].split(".")[0], "%m%d%y"),
        reverse=True,
    )
    ...
    while True:
        try:
            choice = input(
                "Enter the number of the file you want to download (Hit Enter for latest, or 0 to cancel) [1]: "
            ).strip()
            if not choice:
                choice = 1
            else:
...
def check_binary():
    ...
    local_files = [
        f
        for f in os.listdir(binary_folder)
        if os.path.isdir(os.path.join(binary_folder, f)) and date_pattern.match(f)
    ]

    if not local_files:
        print(
            "No binaries found in the Binary folder. Downloading the latest binary..."
        )
        download_latest_build()
        return get_newest_binary_path(binary_folder, current_os)
